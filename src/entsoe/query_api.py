...
def query_core(params: dict, timeout: int = 5):
    ...
    logger.debug(f"Making API request to {URL} with params: {sanitized_params}, timeout: {timeout}")
    
    response = get(URL, params=params, timeout=timeout)
    
    logger.debug(f"API response status: {response.status_code}, content length: {len(response.text) if response.text else 0}")
...
def parse_response(response):
    ...
    logger.debug(f"Extracted namespace: {name}, matching class: {matching_class.__name__ if matching_class else None}")
...
def query_api(params: dict, timeout: int = 5):
    sanitized_params = _sanitize_params_for_logging(params)
    logger.debug(f"Starting query_api with params: {sanitized_params}, timeout: {timeout}")
